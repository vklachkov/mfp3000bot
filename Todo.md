...
- [ ] Починить кнопку остановки сканирования.
    Сейчас она не работает должным образом из-за того, что всё сканирование происходит при первом вызове sane_read.
...
- [x] Для SANE можно отключить получение списка устройств перед открытием.
    Это должно значительно ускорить начало сканирования. Список используется только для поиска устройства по имени.

Рефакторинг

- [x] Отрефакторить simple-cups.
    - [x] Убрать cstr из публичного API.
        - [x] Особенно из настроек. Количество копий не должно передаваться через CStr. 
    - [x] Сделать отдельные типы для различных строк, убрать конвертацию String -> CString из логики.
    - [x] Открытие устройства по имени, без предварительного получения списка устройств.
- [x] Разделить simple-cups на cups и cups-sys.
    - [x] Проверять версию sane через pkg-config.
- [x] Разделить simple-sane на sane и sane-sys.
    - [ ] Проверять версию sane через pkg-config.
- [ ] Перевести проект на русский или английский язык, вместо солянки.